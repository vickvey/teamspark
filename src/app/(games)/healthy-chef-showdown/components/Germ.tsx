...
import { Germ, GermVariant } from '../types';
...
const GermComponent: React.FC<GermProps> = ({ germ, onBust }) => {
  ...
  const animationClass = germ.variant === 'pink' ? 'animate-wiggle animation-duration-500' : 'animate-wiggle';

  return (
    <div
      className={`absolute cursor-pointer transition-transform duration-200 hover:scale-110 ${isBusted ? 'animate-pop' : ''}`}
      style={{ top: `${germ.y}%`, left: `${germ.x}%` }}
      onClick={handleClick}
    >
      <variantDetails.Icon className={`${variantDetails.size} ${animationClass}`} />
    </div>
  );
};
