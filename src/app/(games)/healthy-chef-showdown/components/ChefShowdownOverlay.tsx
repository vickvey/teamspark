...
const ResultDisplay: React.FC<ResultDisplayProps> = ({
  ...
  return (
    <div className="text-center">
      <h2 className={`text-7xl font-bold drop-shadow-lg ${textColor}`}>
        {message}
      </h2>
      ...
      <button
        onClick={onStartGame}
        className="mt-8 bg-yellow-400 hover:bg-yellow-500 text-gray-800 font-bold py-4 px-10 rounded-full text-3xl transition-transform transform hover:scale-105 shadow-lg"
      >
        Play Again
      </button>
    </div>
...
const ChefShowdownOverlay: React.FC<ChefShowdownOverlayProps> = ({
  gameState,
  winner,
  playerScore,
  aiScore,
  ...
  return (
    <div className="absolute inset-0 bg-black bg-opacity-70 flex flex-col justify-center items-center z-30 p-4">
      {gameState === "start" && (
        <div className="text-center">
          <h1 className="text-6xl md:text-8xl text-orange-400 drop-shadow-lg">
            Healthy Chef Showdown
          </h1>
          <p className="text-xl md:text-2xl text-white mt-4 max-w-lg">
            Drag healthy ingredients to your dish to get the highest score. Beat
            the AI Chef!
          </p>
          <button
            onClick={onStartGame}
            className="mt-8 bg-green-500 hover:bg-green-600 text-white font-bold py-4 px-10 rounded-full text-3xl transition-transform transform hover:scale-105 shadow-lg"
          >
            Start Cooking!
          </button>
        </div>
